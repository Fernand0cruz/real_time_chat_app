...
onMounted(async () => {
    ...
    .listen('.sendMessage', async (e) => {
        if (userActive.value && userActive.value.id === e.message.from) {
            await messages.value.push(e.message);
            scrollToBottom();
        } else {
            const userFound = Object.values(users.value).find(user => user.id === e.message.from);

            if (userFound) {
                userFound.notification = true; 
            }
        }
    });
...
const loadMessages = async (userId) => {
    ...
    const userFound = Object.values(users.value).find(user => user.id === userId);
        
    if (userFound) {
        userFound.notification = false; 
    }
    ...
    <AppLayout title="Dashboard">
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="flex w-full bg-white overflow-hidden shadow-xl sm:rounded-lg"
                    style="min-height: 500px; max-height: 500px; width: 100%;">
                    ...
                    <div class="w-9/12 flex flex-col justify-between">
                        <div class="w-full h-full p-6 flex flex-col overflow-y-scroll">
                            <div v-for="message in messages" :key="message.id"
                                ...
                                <p :class="(message.from === $attrs.auth.user.id ? 'messageFromMe' : '')"
                                    class="inline-block p-2 rounded-md bg-gray-200 max-w-[75%]">
                                    {{ message.content }}
                                </p>
                                <span class="block mt-1 text-xs text-gray-500">{{ $formatDate(message.created_at)
                                    }}</span>
                            </div>
                        </div>
                            ...
                            <form v-on:submit.prevent="sendMessage">
                                <div class="flex">
                                    <input v-model="message" type="text"
                                        class="flex-1 px-4 py-2 text-sm focus:outline-none" autocomplete="off" />
                                    <button type="submit"
